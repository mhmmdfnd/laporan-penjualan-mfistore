...
info TEXT,
kode_cadangan JSONB DEFAULT '[]'::jsonb,
metode_beli TEXT,
tempat TEXT,
tanggal_jual TEXT,
...
diterima BIGINT DEFAULT 0,
keuntungan BIGINT DEFAULT 0,
hackback BOOLEAN DEFAULT FALSE,
tanggal_hackback TEXT,
bukti_status TEXT DEFAULT 'belum',
created_at TIMESTAMPTZ DEFAULT NOW()
);`);

await pool.query(`
ALTER TABLE sales
ADD COLUMN IF NOT EXISTS bukti_status TEXT DEFAULT 'belum';
`);

await pool.query(`
UPDATE sales
SET bukti_status='belum'
WHERE bukti_status IS NULL OR bukti_status='';
`);
...
function mapSale(x){
  return {
    ...x,
    id:Number(x.id),
    harga_beli:Number(x.harga_beli)||0,
    harga_jual:Number(x.harga_jual)||0,
    potongan:Number(x.potongan)||0,
    diterima:Number(x.diterima)||0,
    keuntungan:Number(x.keuntungan)||0,
    garansi:Number(x.garansi)||0,
    kode_cadangan:Array.isArray(x.kode_cadangan)?x.kode_cadangan:[],
    bukti_status:['belum','screenshot','upload'].includes(x.bukti_status)
      ? x.bukti_status
      : 'belum'
  };
}
app.get('/api/sales',auth,async(req,res)=>{try{await init();const r=await pool.query('SELECT * FROM sales ORDER BY id DESC');const q=String(req.query.q||'').toLowerCase(),
status=String(req.query.status||''),
jenis=String(req.query.jenis||''),
bukti=String(req.query.bukti||''),
from=String(req.query.from||''),
to=String(req.query.to||'');
const rows=r.rows.map(mapSale).filter(x=>{const hay=[x.kode,x.jenis,x.info,...x.kode_cadangan,x.pembeli,x.pesanan,x.tempat].join(' ').toLowerCase();
const st=x.hackback?'HB':(x.tanggal_jual?'Sold':'Available');
const d=x.hackback?(x.tanggal_hackback||x.tanggal_jual||''):(x.tanggal_jual||'');
return(!q||hay.includes(q))
&&(!status||status===st)
&&(!jenis||jenis===x.jenis)
&&(!bukti||bukti===x.bukti_status)
&&(!from||d>=from)
&&(!to||d<=to);});res.json(rows);}catch(e){res.status(500).json({error:e.message});}});
...
app.put('/api/sales/:id/bukti',auth,async(req,res)=>{
  try{
    await init();
    ...
    const allowed=['belum','screenshot','upload'];

    if(!allowed.includes(status)){
      return res.status(400).json({
        error:'Status bukti tidak valid'
      });
    }
    ...
    res.json({
      ...
      bukti_status:r.rows[0].bukti_status
    });
